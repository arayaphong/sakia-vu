...
private:
    ...
    // bar top over a column, step to the shorter neighbor in a gap slot; false
    // outside the meter.
    bool surfaceYAt(float lx, float& surfaceY) const;
    // Convex box filling gap slot `gap`: flat top at the shorter neighbor's
    // level, leaving a vertical cliff against the taller bar, bottom edge 1 m
    // below the ground line (never degenerate).
