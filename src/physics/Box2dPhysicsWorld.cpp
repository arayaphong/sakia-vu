...
void Box2dPhysicsWorld::enforceSurface() {
    ...
    for (auto& [body, obj] : objects_) {
        ...
        // that point's own x, because a rotated box's lowest corner is offset
        // from its center; measuring at the center x would mis-judge depth for
        // tilted boxes.
...
void Box2dPhysicsWorld::addObject(PhysicsObject::Kind kind, float lx, float ly) {
    ...
    sd.material.restitution = isBall ? 0.3f : 0.2f;
    if (isBall) {
